...
func TestIsProhibitedContent(t *testing.T) {
	cases := []struct {
		...
		{"gemini-safety-finish", `{"candidates":[{"finishReason":"SAFETY"}]}`, true},
		{"gemini-recitation-finish", `{"candidates":[{"finishReason":"RECITATION"}]}`, true},
		{"gemini-blocklist-finish", `{"candidates":[{"finishReason":"BLOCKLIST"}]}`, true},
		{"gemini-image-safety-finish", `{"candidates":[{"finishReason":"IMAGE_SAFETY"}]}`, true},
		{"gemini-prompt-feedback", `{"promptFeedback":{"blockReason":"PROHIBITED_CONTENT"}}`, true},
		{"gemini-safety-prompt-feedback", `{"promptFeedback":{"blockReason":"SAFETY"}}`, true},
		{"gemini-stop", `{"candidates":[{"finishReason":"STOP"}]}`, false},
